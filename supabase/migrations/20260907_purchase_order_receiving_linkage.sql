-- Receiving stock against a purchase order only ever linked the receipt's
-- header (stock_receipts.purchase_order_id) — nothing attributed a receipt
-- LINE to the purchase_order_line it fulfilled, so purchase_order_lines
-- .quantity_received stayed 0 forever and the order's status never moved
-- past draft/sent, even after everything on it had been received. This
-- adds that linkage: each receipt line can now carry a
-- purchase_order_line_id, and completing the receipt rolls the received
-- quantity and the order's status (partially_received/received) up
-- atomically in the same transaction as everything else.
--
-- The signature gains p_purchase_order_id, so this is a DROP + CREATE
-- rather than a CREATE OR REPLACE (Postgres treats a changed argument
-- list as a distinct function). This also folds in the SECURITY DEFINER
-- fix from 20260907_procurement_rpc_security_definer.sql — that migration
-- targets the old 7-arg signature being dropped here, so it's restated
-- fresh below rather than relied on.

DROP FUNCTION IF EXISTS create_completed_stock_receipt(
  UUID, UUID, UUID, TIMESTAMPTZ, UUID, TEXT, JSONB
);

CREATE FUNCTION create_completed_stock_receipt(
  p_store_id UUID,
  p_supplier_id UUID,
  p_branch_id UUID,
...
BEGIN
  ...
  -- (e.g. all-rejected lines) didn't actually receive anything.
