-- Migration: 20260211_prioritize_crm_sources.sql
-- Update CRM view to prioritize Stores/Events/Circles/Publications OVER Manual entries
-- "Manual" will still show if it is the ONLY source, or if the user explicitly prefers manual over others? 
-- No, the request is to fix "Source" display so manual doesn't hide the others.

-- 1. DROP VIEW IF EXISTS
DROP VIEW IF EXISTS crm_contacts_unified CASCADE;

-- 2. RECREATE VIEW with Priority Sorting
-- We add a 'priority' column for sorting:
-- Store: 1, Event: 2, Circle: 3, Publication: 4, Manual: 5
-- Then we sort by priority ASC, then updated_at DESC
...
ORDER BY business_id, email, priority ASC, last_contact_at DESC;
