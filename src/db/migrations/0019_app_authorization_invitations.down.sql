...
revoke insert, delete on app.role_permissions from scripe_app;

drop policy if exists roles_manage on app.roles;
revoke insert, update, delete on app.roles from scripe_app;

drop policy if exists membership_roles_manage on app.membership_roles;
revoke insert, delete on app.membership_roles from scripe_app;

drop policy if exists business_memberships_manage on app.business_memberships;
revoke update on app.business_memberships from scripe_app;
