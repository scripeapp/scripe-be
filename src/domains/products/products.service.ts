...
export class ProductsService {
  ...
  async listModifierGroups(operation: ProductOperation, storeId: string): Promise<ModifierGroup[]> { return this.run(operation, async (c) => (await repository.listModifierGroups(c, operation.businessId, storeId)).map(mapModifierGroup)); }
  async getModifierGroup(operation: ProductOperation, groupId: string): Promise<ModifierGroupDetail> { return this.run(operation, async (c) => { const row = await repository.findModifierGroup(c, operation.businessId, groupId); if (!row) throw notFoundError("Modifier group not found"); const options = (await repository.listModifierOptions(c, operation.businessId, groupId)).map(mapModifierOption); return { ...mapModifierGroup(row), options }; }); }
  async createModifierGroup(operation: ProductOperation, input: ModifierGroupInput): Promise<ModifierGroup> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); const row = await repository.createModifierGroup(c, operation.businessId, input); return mapModifierGroup({ ...row, optionsCount: 0, productCount: 0 }); }); }
  async updateModifierGroup(operation: ProductOperation, groupId: string, input: ModifierGroupUpdateInput): Promise<ModifierGroup> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); const updated = await repository.updateModifierGroup(c, operation.businessId, groupId, input); if (!updated) throw notFoundError("Modifier group not found"); const row = await repository.findModifierGroup(c, operation.businessId, groupId); return mapModifierGroup(row!); }); }
  async archiveModifierGroup(operation: ProductOperation, groupId: string): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); if (!(await repository.archiveModifierGroup(c, operation.businessId, groupId))) throw notFoundError("Modifier group not found"); }); }
  async reorderModifierGroups(operation: ProductOperation, storeId: string, orderedIds: readonly string[]): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); await repository.reorderModifierGroups(c, operation.businessId, storeId, orderedIds); }); }
  async createModifierOption(operation: ProductOperation, groupId: string, input: ModifierOptionInput): Promise<ModifierOption> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); return mapModifierOption(await repository.createModifierOption(c, operation.businessId, groupId, input)); }); }
  async updateModifierOption(operation: ProductOperation, groupId: string, optionId: string, input: ModifierOptionUpdateInput): Promise<ModifierOption> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); const row = await repository.updateModifierOption(c, operation.businessId, groupId, optionId, input); if (!row) throw notFoundError("Modifier option not found"); return mapModifierOption(row); }); }
  async archiveModifierOption(operation: ProductOperation, groupId: string, optionId: string): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); if (!(await repository.archiveModifierOption(c, operation.businessId, groupId, optionId))) throw notFoundError("Modifier option not found"); }); }
  async reorderModifierOptions(operation: ProductOperation, groupId: string, orderedIds: readonly string[]): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "modifier.manage"); await repository.reorderModifierOptions(c, operation.businessId, groupId, orderedIds); }); }
  async listProductModifierGroups(operation: ProductOperation, productId: string): Promise<AttachedModifierGroup[]> { return this.run(operation, async (c) => repository.listProductModifierGroups(c, operation.businessId, productId)); }
  async attachModifierGroup(operation: ProductOperation, productId: string, groupId: string): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "product.update"); await repository.attachModifierGroup(c, operation.businessId, productId, groupId); }); }
  async detachModifierGroup(operation: ProductOperation, productId: string, groupId: string): Promise<void> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "product.update"); await repository.detachModifierGroup(c, operation.businessId, productId, groupId); }); }
...
function mapModifierGroup(row: ModifierGroupRow & { optionsCount: number; productCount: number }): ModifierGroup { return { ...row, createdAt: row.createdAt.toISOString(), updatedAt: row.updatedAt.toISOString() }; }
function mapModifierOption(row: ModifierOptionRow): ModifierOption { return { ...row, createdAt: row.createdAt.toISOString() }; }
