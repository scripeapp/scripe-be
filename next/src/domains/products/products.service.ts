...
export class ProductsService {
  ...
  async create(operation: ProductOperation, input: ProductCreateInput): Promise<Product> { return this.run(operation, async (c) => { await this.require(c, operation.businessId, "product.create"); const created = await repository.createProduct(c, operation.businessId, operation.userId, input, input.slug ?? `${slugify(input.name)}-${randomUUID().slice(0, 8)}`); return this.hydrate(c, created); }); }
