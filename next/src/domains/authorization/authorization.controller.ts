...
export class AuthorizationController {
  ...
  readonly listMembers = this.handle(async (request) => {
    ...
    return { members: await this.service.listMembers(this.operation(request, businessId)) };
  });

  readonly getMyMembership = this.handle(async (request) => {
    const { businessId } = schemas.businessParamsSchema.parse(request.params);
    return { membership: await this.service.getMyMembership(this.operation(request, businessId)) };
  });

  readonly setMemberRoles = this.handle(async (request) => {
    const { businessId, membershipId } = schemas.membershipParamsSchema.parse(request.params);
    return {
  ...
  readonly removeMember = this.handle(async (request) => {
    const { businessId, membershipId } = schemas.membershipParamsSchema.parse(request.params);
    await this.service.removeMember(this.operation(request, businessId), membershipId);
    return { removed: true };
  });
  ...
  readonly listPendingInvitations = this.handle(async (request) => {
    const { businessId } = schemas.businessParamsSchema.parse(request.params);
    return { invitations: await this.service.listPendingInvitations(this.operation(request, businessId)) };
  });

  readonly revokeInvitation = this.handle(async (request) => {
    const { businessId, invitationId } = schemas.invitationParamsSchema.parse(request.params);
    await this.service.revokeInvitation(this.operation(request, businessId), invitationId);
    return { revoked: true };
  });

  readonly acceptInvitation = this.handle(async (request) => {
    const { token } = schemas.acceptInvitationParamsSchema.parse(request.params);
    return {
      accepted: await this.service.acceptInvitation(
        { userId: requireAuthContext(request).userId, requestId: request.requestId },
        token,
      ),
    };
  }, 201);
  ...
  readonly listRoles = this.handle(async (request) => {
    const { businessId } = schemas.businessParamsSchema.parse(request.params);
    return { roles: await this.service.listRoles(this.operation(request, businessId)) };
  });

  readonly getRole = this.handle(async (request) => {
    const { businessId, roleId } = schemas.roleParamsSchema.parse(request.params);
    return { role: await this.service.getRole(this.operation(request, businessId), roleId) };
  });

  readonly createRole = this.handle(async (request) => {
    const { businessId } = schemas.businessParamsSchema.parse(request.params);
    return {
      role: await this.service.createRole(this.operation(request, businessId), schemas.createRoleSchema.parse(request.body)),
    };
  }, 201);
  ...
  readonly updateRole = this.handle(async (request) => {
    const { businessId, roleId } = schemas.roleParamsSchema.parse(request.params);
    return {
      role: await this.service.updateRole(this.operation(request, businessId), roleId, schemas.updateRoleSchema.parse(request.body)),
    };
  });
  ...
  private operation(request: Request, businessId: string): AuthorizationOperation {
    return {
      userId: requireAuthContext(request).userId,
      businessId,
      requestId: request.requestId,
