...
export class PricingService {
  ...
  createPrice(o: PricingOperation, input: PriceInput): Promise<Price> { return this.run(o, async (c) => { await this.require(c, "pricing.manage"); return mapPrice(await repo.createPrice(c, o.businessId, o.userId, input)); }); }
  archivePrice(o: PricingOperation, id: string): Promise<void> { return this.run(o, async (c) => { await this.require(c, "pricing.manage"); if (!(await repo.archivePrice(c, o.businessId, id))) throw notFoundError("Price not found"); }); }
  ...
  setLocation(o: PricingOperation, input: LocationSettingInput): Promise<LocationSetting> { return this.run(o, async (c) => { await this.require(c, "pricing.manage"); return repo.upsertLocationSetting(c, o.businessId, input); }); }
  ...
  createTaxRate(o: PricingOperation, input: Parameters<typeof repo.createTaxRate>[2]): Promise<TaxRate> { return this.run(o, async (c) => { await this.require(c, "pricing.manage"); return mapTax(await repo.createTaxRate(c, o.businessId, input)); }); }
